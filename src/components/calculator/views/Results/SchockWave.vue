...
<template>    
    <v-expansion-panel  class="result">
        ...
        <v-expansion-panel-content>
            ...
            <div class="result_effect"><span class="results_effects_name">{{$t('calculator.results.shockwave.Maximum wind speed')}}: </span>&nbsp;{{round1(shock_wave.point_assesment.max_wind_speed)}} {{$t('calculator.dimensions.m/s')}}</div>
        </v-expansion-panel-content>
    </v-expansion-panel>
</template>
...
/*     created()
    {
        console.log("Shock wave results created", this.$dimension_prefix_format(1230));
    } */

    round_digits:number = 3;
    round(val: number): string{
        //return val.toPrecision(this.round_digits);
        return '' + math_ext.round_by_digits_to_string(val, this.round_digits);
    }
    round1(val: number): string{
        //return val.toPrecision(this.round_digits);
        return '' + math_ext.round_by_digits_to_string(val, 1);
    }
}
</script>
